...
class DependdentData:
	...
	#执行依赖测试，获取结果
	def run_dependent(self,head=None):
		...
		res = run_method.run_main(method,url,request_data,head)
		return json.loads(res)

	#根据依赖的key去获取执行依赖测试case的响应,然后返回
	def get_data_for_key(self,row,head=None):
		depend_data = self.data.get_depend_key(row)
		response_data = self.run_dependent(head)
